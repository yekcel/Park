...
@section('content')
    ...
    <div class="card bg-light text-dark ">
        <div class="card-header">
            <div class="row">
                <div class="col-sm-10">گزارش عملکرد هزینه ای برای شرکت {{$company->name}}</div>
        ...
        <div class="card-body ">
            <div class="card bg-secondary text-white ">
                ...
                <div class="card-body ">
                    <form action="{{'/reports/spent_company'}}" method="post" class="form-inline">
                        {{ csrf_field() }}
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-sm3">
                                    <div class="form-group" style="padding-top: 10px">
                                        ...
                                        <select name="company" class="form-control" id="company">
                                            <option value="">لطفا شرکت را انتخاب نمایید...</option>
                                            @foreach($companies as $company1)
                                                <option value="{{$company1->id}}" @if($company1->id==$company->id) selected @endif>{{$company1->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tr>
                        ...
                        <th class="text-center">مبلغ کل هزینه(ریال)</th>
                        <th class="text-center">سهم واحد از هزینه(ریال)</th>
                        <th class="text-center">محل مصرف</th>
                        <th class="text-center">کد محل مصرف</th>
                        <th class="text-center">اطلاعات هزینه</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php $sum_spent=0;?>
                    ...
                    @foreach($spents as $spent)
                        ...
                        <tr class="text-center">
                            ...
                            <td>{{number_format($spent->price)}}</td>
                            <?php
                            $sahm_price=$spent->price/$spent->company_count;
                            $sum_spent= $sum_spent+$sahm_price ?>
                            <td>{{number_format($sahm_price)}}</td>
                            <td>{{$spent->credit?$spent->credit->name:null}}</td>
                            <td>{{$spent->credit?$spent->credit->credit_code:null}}</td>
                            <td>

                                <button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#exampleModalLong<?php echo $spent->id; ?>">نمایش هزینه
                                </button>
                                <!-- Modal -->
                                <div class="modal fade" id="exampleModalLong<?php echo $spent->id; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLongTitle">اطلاعات هزینه</h5>
                                            ...
                                            <div class="modal-body">
                                                <table class="table table-bordered">
                                                    <tbody>
                                                    <tr>
                                                        <td style="width: 30%">تاریخ هزینه:</td>
                                                    ...
                                                    <tr>
                                                        <td>فعالیت:</td>
                                                        <td>{{$spent->subaction->action->name}}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>ریزفعالیت:</td>
                                                        <td>{{$spent->subaction->action->application->name}}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>عنوان هزینه:</td>
                                                        <td>{{$spent->name}}</td>
                                                        ...
                                                        <td>واحد های ذینفع:</td>
                                                        <td>
                                                            @foreach($spent->company as $company2)
                                                                {{$company2->name}}
                                                                <br>
                                                                @endforeach
                                                           </td>
